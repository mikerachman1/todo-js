  ...
  function addTaskListeners() {
    Object.values(tasks).forEach((task) => {
      ...
      editButton.addEventListener('click', function() {
        taskDiv.innerHTML = `<form id="edit-task-${task.id}">
                              <label for="title">
        ...
        editTaskForm.addEventListener("submit", 
          function(event) {
            ...
            taskDiv.innerHTML = `<span class='task-details-${task.id}'>${task.title} - ${task.description} - ${format((task.dueDate), 'M/dd/yy')} - ${task.priority}</span>
                                <input type="checkbox" class='task-checkbox-${task.id}'>
                                <button class='task-edit-${task.id}'>Edit</button>
                                <button class='task-delete-${task.id}'>Delete</button>`
            addTaskListeners();
